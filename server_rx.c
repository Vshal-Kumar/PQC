...
static tracked_client_t *find_or_add_tracked(int cid, const char *ip,
                                             uint16_t port) {
  for (int i = 0; i < g_tracked_count; i++)
    if (g_tracked[i].client_id == cid)
      return &g_tracked[i];
  if (g_tracked_count >= MAX_TRACKED)
    return NULL;
...
static void render_rx_block(const ipc_rx_payload_t *info, const uint8_t *msg,
                            uint32_t msg_len, const ipc_metrics_rx_t *metrics) {
  char ts[20];
  ...
  printf("%s║%s  %s%s:%u%s  │  SID: %016llx  │  Msg #%u\n", col, CLR_RESET,
         ...
         (unsigned long long)info->session_id, msg_seq);

  /* ── Message content ────────────────────────────────────────── */
  printf("%s╠══ MESSAGE %s\n", col, CLR_RESET);
  printf("%s║  %s%.*s%s\n", col, CLR_WHITE, (int)msg_len, (const char *)msg,
         CLR_RESET);
  ...
  if (metrics) {
    ...
    printf("%s║%s  Decrypt    : %s%.1f µs%s  avg %.1f µs\n", col, CLR_RESET,
           CLR_GREEN, metrics->decrypt_us, CLR_RESET, metrics->avg_decrypt_us);

    if (metrics->pkt_seq > 1) {
      printf(
          "%s║%s  Inter-arr  : %s%.3f ms%s  jitter %.3f ms  (RFC 3550 EWMA)\n",
          col, CLR_RESET, CLR_YELLOW, metrics->interarrival_ms, CLR_RESET,
          metrics->jitter_ms);
    } else {
    ...
    printf("%s║%s  Bytes recv : %llu B  │  Pkt seq: #%u\n", col, CLR_RESET,
           (unsigned long long)metrics->bytes_recv, metrics->pkt_seq);
  }
...
static void render_connect_event(int cid, const char *ip, uint16_t port,
                                 int connected) {
  char ts[20];
  now_hms(ts, sizeof(ts));
  const char *col = client_color(cid);
  ...
  if (g_tracked_count == 0) {
    ...
    for (int i = 0; i < g_tracked_count; i++) {
      printf("║  CLIENT-%02d (%s:%u) : Received %u messages\n",
             g_tracked[i].client_id, g_tracked[i].ip_str, g_tracked[i].port,
             g_tracked[i].msg_count);
    }
  }
  printf("╚══════════════════════════════════════════════╝\n");
}
...
int main(void) {
  ...
  while (!g_stop) {
    ...
    case IPC_MSG_RX: {
      if (n < (ssize_t)sizeof(ipc_rx_payload_t))
        break;
    ...
    case IPC_METRICS_RX: {
      if (n < (ssize_t)sizeof(ipc_metrics_rx_t))
        break;
    ...
    case IPC_CLIENT_CONNECT: {
      if (n < (ssize_t)sizeof(ipc_client_entry_t))
        break;
